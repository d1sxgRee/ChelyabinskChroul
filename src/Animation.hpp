...
private:
    ...
    double endX;     //конечна€ длина одного спрайтшита
    double loop_x;   //позици€ начала цикла на спрайте по х
    HDC image;       //указатель на спрайтшит
    COLORREF color;  //удал€емый цвет со спрайта
public:
    ...
    image(_image), color(_color)
{
    if(image==nullptr)
    {
        txMessageBox("Ќе найден файл со спрайтом", "ќшибка");
        exit(0);
    }
}
